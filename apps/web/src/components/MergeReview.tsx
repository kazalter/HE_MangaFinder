import type { AgentStatus, MergeSuggestion } from '../types'
...
export function MergeReview({ suggestions, agentStatus, busy, onRunAgent, onAccept, onReject, onClose }: {
      ...
      <section className="merge-review" role="dialog" aria-modal="true" aria-label="聚合候选审核">
        ...
        <div className="suggestion-list">
          {suggestions.map((item) => (
            <article key={item.id}>
              <div><strong>{item.source_title}</strong><span>可能是同一部</span><strong>{item.target_title}</strong></div>
              <p>{item.reasons.join(' · ')} · 置信度 {Math.round(item.confidence * 100)}%</p>
              ...
              <div><button disabled={busy} onClick={() => onReject(item.id)}>保持分开</button><button disabled={busy} onClick={() => onAccept(item.id)}>确认合并</button></div>
            </article>
          ))}
